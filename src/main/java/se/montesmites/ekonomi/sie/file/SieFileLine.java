...
public class SieFileLine {
  ...
  static SieFileLine of(String line) {
    return switch (line.trim()) {
      case "{" -> new SieFileLine(SieFileLineType.BEGIN_SUBRECORDS, line);
      case "}" -> new SieFileLine(SieFileLineType.END_SUBRECORDS, line);
      default -> new SieFileLine(SieFileLineType.PROSAIC, line);
    };
  }
