...
import static se.montesmites.ekonomi.report.xml._Definition4TestUtil.definition;
import static se.montesmites.ekonomi.report.xml._Definition4TestUtil.row;
import static se.montesmites.ekonomi.report.xml._Definition4TestUtil.section;
...
class DefinitionTest {
  ...
  void t01_oneSectionOneRowSimpleDefinition() {
    ...
    var definition =
        definition(
            "One Section One Row Simple Definition",
            of(section("Section 1", of(row("Description 1-1", "Regex 1-1")))));
    doAssert(definition);
  }
  ...
  void t02_oneSectionTwoRowsSimpleDefinition() {
    ...
    var definition =
        definition(
            "One Section Two Rows Simple Definition",
            of(
                section(
                    "Section 1",
                    of(row("Description 1-1", "Regex 1-1"), row("Description 1-2", "Regex 1-2")))));
    doAssert(definition);
  }
  ...
  void t03_twoSectionsOneRowEachSimpleDefinition() {
    var path =
        "/se/montesmites/ekonomi/report/xml/03_two-sections-one-row-each-simple-definition.xml";
    this.xmlDefinition = JAXB.unmarshal(getClass().getResourceAsStream(path), XmlDefinition.class);
    this.reportBuilder = xmlDefinition.toReportBuilder(organization, year);
    var definition =
        definition(
            "Two Sections One Row Each Simple Definition",
            of(
                section("Section 1", of(row("Description 1-1", "Regex 1-1"))),
                section("Section 2", of(row("Description 2-1", "Regex 2-1")))));
    doAssert(definition);
  }
  ...
  void t06_twoSectionRefsOneCompact() {
    var path =
        "/se/montesmites/ekonomi/report/xml/06_two-sectionrefs-one-compact-ref-one-compact-section.xml";
    this.xmlDefinition = JAXB.unmarshal(getClass().getResourceAsStream(path), XmlDefinition.class);
    this.reportBuilder = xmlDefinition.toReportBuilder(organization, year);
    var definition =
        definition(
            "Two SectionRefs, One Compact Ref, One Compact Section",
            of(
                section(
                    "Section 1",
                    of(CompactSectionDecorator.class),
                    of(row("Description 1-1", "Regex 1-1"))),
                section(
                    "Section 2",
                    of(CompactSectionDecorator.class),
                    of(row("Description 2-1", "Regex 2-1")))));
    doAssert(definition);
  }

  private void doAssert(_ReportDefinition4Test definition) {
    assertEquals(
        definition.getDescription(), reportBuilder.getDescription(), "definition description");
    ...
    for (var section : definition.getSections()) {
      ...
      assertEquals(
          ...
          getSectionBuilderAt(sectionIndex).getDescription(),
          sectionMessage + ", description");
      assertEquals(
          section.getRows().size(),
          getBodyRowBuildersAt(sectionIndex).size(),
          sectionMessage + ", row count");
      assertEquals(
          section.getDecorators(),
          getSectionBuilderAt(sectionIndex)
              .getDecorators()
              .stream()
              .map(SectionDecorator::getClass)
              .collect(toList()));
      var rowIndex = 0;
      for (var row : section.getRows()) {
        ...
        assertEquals(
            row.getDescription(),
            getBodyRowBuilderAt(sectionIndex, rowIndex).getDescription(),
            rowMessage + ", description");
        assertEquals(
            row.getRegex(),
            getRegexPatternAt(sectionIndex, rowIndex),
            rowMessage + ", regex pattern");
        rowIndex++;
      }
      sectionIndex++;
    }
